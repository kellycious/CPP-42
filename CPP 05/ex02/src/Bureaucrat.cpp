...
/*   Updated: 2023/10/15 17:08:10 by khuynh           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void Bureaucrat::executeForm( AForm const &form ) const
{
	try
	{
		form.requirementscheck(*this);
		std::cout << this->_name << " executes " << form.getName() << " form." << std::endl;
		form.execute(*this);
	}
	catch (std::exception &e)
	{
		std::cout << "\e[0;31m" << this->_name << " cannot execute " << form.getName() << " because " << e.what() << "\033[0m" << std::endl;
	}
};
